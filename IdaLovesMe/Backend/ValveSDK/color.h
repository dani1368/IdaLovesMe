...
public:
	...
	{
		float red_diff = float(fabs(Base(RGBA[0]) - Base(color.RGBA[0])));
		float green_diff = float(fabs(Base(RGBA[1]) - Base(color.RGBA[1])));
		float blue_diff = float(fabs(Base(RGBA[2]) - Base(color.RGBA[2])));
		float alpha_diff = float(fabs(Base(RGBA[3]) - Base(color.RGBA[3])));
	...
	/*
	static CColor FromHSB(float hue,
		float saturation ,
		float brightness,
		int alpha = 255)
	{
